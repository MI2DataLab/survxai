...
```{r, fig.height=6}
plot(cp_cph, scale_type = "gradient", col_scale = c("red", "blue"), ncol = 2)
```

We can see that there are  differences for stages. Next, we will plot Ceteris Paribus for sigle variable `stage`.

```{r, fig.height=3}
plot(cp_cph, selected_variable = "stage", scale_type = "gradient", col_scale = c("red", "blue"))
```
...
```{r}
plot(broken_prediction_cph, col_scale = c("red", "blue"))
```
